...
int main() {
  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = (hit0_brdf*hit0_refl.dot(hit0_normal)*hit1_rad)/(nsample*hit0_pdf);
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float rad = (hit0_brdf*hit0_refl.dot(hit0_normal)*hit1_rad)/(num_half_sample*(hit0_pdf_light_sample + hit0_pdf_brdf_sample));
  ...
  output_float_image(
      (std::experimental::filesystem::path(PROJECT_SOURCE_DIR) / "out_brdf.png").string().c_str(),
      img_width, img_height, img_brdf);
  output_float_image(
      (std::experimental::filesystem::path(PROJECT_SOURCE_DIR) / "out_light.png").string().c_str(),
      img_width, img_height, img_light);
  output_float_image(
      (std::experimental::filesystem::path(PROJECT_SOURCE_DIR) / "out_mis.png").string().c_str(),
